...
while [ $SECONDS_PASSED -lt $MAX_WAIT_TIME ]; do
  ...
  if [ ! -z "$PID" ]; then
    ...
    -o "expression -l objc -- id engine = [EligibilityEngine sharedInstance]; NSArray<NSString*> *sels = @[@"_onQueue_recomputeAllDomainAnswers", @"recomputeAllDomainAnswers"]; for (NSString *selName in sels) { SEL sel = NSSelectorFromString(selName); if ([engine respondsToSelector:sel]) { (void)[engine performSelector:sel]; NSLog(@"%@ performed", selName); break; } } " \
    -o "process detach" \
    -o quit
