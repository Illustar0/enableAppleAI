...
while [ $SECONDS_PASSED -lt $MAX_WAIT_TIME ]; do
  ...
  if [ ! -z "$PID" ]; then
    ...
    -o "expression @import Foundation; id engine = [EligibilityEngine sharedInstance]; NSArray *sels = @[@\"_onQueue_recomputeAllDomainAnswers\", @\"recomputeAllDomainAnswers\"]; for (NSString *selName in sels) { SEL sel = NSSelectorFromString(selName); if ([(id)engine respondsToSelector:sel]) { (void)[(id)engine performSelector:sel]; NSLog(@\"%@ performed\", selName); break; } }" \
    -o "process detach" \
    -o quit
